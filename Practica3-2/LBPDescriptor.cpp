...
vector<string> LBPDescriptor::LecturaCarpeta(string c){
    vector<string> paths;
    
    for (const auto &file : fs::recursive_directory_iterator(c)){
...
Mat LBPDescriptor::ConversionImagen(Mat imagen){
    Mat image;
    cvtColor(imagen, image, COLOR_BGR2Lab);
    return image;
